package com.footballer.rest.api.v2.dao.interceptor;
...
public class MybatisPagedInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if(statementHandler instanceof RoutingStatementHandler){
			...
			if(StringUtils.isNullOrEmpty(sql) || !sql.toLowerCase().startsWith("select")){
				return invocation.proceed();
			}
	...
	private String buildPagedSql(String sql, int offset,int limit){
		...
		pagedSql.append("select ullll.* from (  ");
		pagedSql.append("select rownum as dpRowNo, ulccc.* from (");
		pagedSql.append(sql);
		pagedSql.append(")  ulccc  where rownum < ").append((offset+limit+1));
		pagedSql.append(") ullll  where ullll.dpRowNo > ").append(offset);
